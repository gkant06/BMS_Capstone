...
dir_path = '/jet/home/thanhngp/BMS1/sample_ASO_all' # contains images from "AmSO4_21223", "HELIX_21221", "MIDAS_Plus_21222"
...
print(input_shape)
# 576 input images of size (512,512,3), 460 train & 116 test
# ========================
#View a few images
plt.figure(1)
...
import pandas as pd
# test_label = np.hstack((x_test, Z, labels))
df = pd.DataFrame({'x_test': list(x_test), 'Z': list(Z), 'label': list(labels)})
df.to_csv('/jet/home/thanhngp/BMS1/combine_test_label.csv')


from sklearn.manifold import TSNE

tsne = TSNE(n_components=2)
mu_tsne = tsne.fit_transform(Z)

#Plot dim1 and dim2 for mu
plt.figure(figsize=(10, 10))
# plt.scatter(mu[:, 0], mu[:, 1], c=labels, cmap='brg')
plt.scatter(mu_tsne[:, 0], mu_tsne[:, 1], c=labels, cmap='brg')
plt.xlabel('dim 1')
plt.ylabel('dim 2')
plt.colorbar()
...
#Take sample from some defined linear space. In this example range [-1, 1]
#Feed it to the decoder and update zeros in the figure with output.


n = 10 # generate 15x15 digits
